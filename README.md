...
author, run, or review `go-eval` suites:
`docs/agent-skills/authoring-go-eval-suites/`.

Claude Code users can invoke the companion `/eval` command from
`.claude/commands/eval.md`. The command infers design, run, or review mode from
repo state and uses the skill's report template and recommendation heuristics.
